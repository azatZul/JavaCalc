...
import ru.kpfu.ivmiit.ooaip.operations.Divide;
import ru.kpfu.ivmiit.ooaip.operations.Multiply;
import ru.kpfu.ivmiit.ooaip.operations.Operation;
import ru.kpfu.ivmiit.ooaip.operations.Plus;
import ru.kpfu.ivmiit.ooaip.operations.Substract;
...
public class ExpressionParser {
	...
	private void getToken() {
		...
		while (explds < exp.length()
				&& Character.isWhitespace(exp.charAt(explds))) {
			++explds;
		}
		// Проверка на окончание выражения
		if (explds == exp.length()) {
	...
	private double evalExp2() throws ParserException {
		...
		while ((op = token.charAt(0)) == '+' || op == '-') {
			...
			switch (op) {
			case '-':
				operation = new Substract(result, partialResult);
				break;
			case '+':
				operation = new Plus(result, partialResult);
				break;
			}
			if (operation != null) {
				result = operation.evualate();
			}
		}
		return result;
	}
	...
	private double evalExp3() throws ParserException {
		...
		while ((op = token.charAt(0)) == '*' || op == '/' | op == '%') {
			...
			switch (op) {
			case '*':
				operation = new Multiply(result, partialResult);
				break;
			case '/':
				if (partialResult == 0.0) {
					handleErr(DIVBYZERO);
				}
				operation = new Divide(result, partialResult);
				break;
			case '%':
				if (partialResult == 0.0) {
					handleErr(DIVBYZERO);
